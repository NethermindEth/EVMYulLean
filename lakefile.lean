...
extern_lib ffi_rs pkg := do
  proc { cmd := "cargo", args := #["build", "--release"],
         cwd := pkg.dir / "EvmYul" / "FFI" / "ffi-rs" } (quiet := true)
  let libName := nameToStaticLib "ffi_rs"
  inputBinFile $ pkg.dir / "EvmYul" / "FFI" / "ffi-rs" / "target" / "release" / libName
